...
namespace DTT.BubbleShooter.Demo
{
    ...
    public class BubbleMovement : MonoBehaviour
    {
        ...
        [SerializeField] private List<AudioClip> ballLandAudioClip;
        [SerializeField] private List<AudioClip> ballBoundAudioClip;
        ...
        //    _currentVelocity = Vector3.Lerp(_currentVelocity, targetVelocity, Time.fixedDeltaTime * _smoothFactor);
        //    _rigidbody.velocity = _currentVelocity * Time.fixedDeltaTime;
        //}
        ...
        private void Update()
        {
            ...
            foreach (RaycastHit2D continuousHit in continuousHits)
            {
                ...
                if (hitController.Bubble == null)
                {
                    ...
                    {
                        Debug.Log("hitController: " + hitController);
                ...
                if (_visitedController.Count > 0)
                {
                    ...
                    //var filteredControllers = _visitedController
                    //    .Where(controller =>
                    //    {
                    //        bool touch = grid.Touch(controller.Position.Value);
                    //        bool contains = hitControllerPositions.Contains(controller.Position.Value);
                    //        Debug.Log($"Position: {controller.Position.Value}, Touch: {touch}, Contains: {contains}");
                    //        return touch && contains;
                    //    }).ToList();

                    //if (!filteredControllers.Any())
                    //{
                    //    Debug.LogWarning("No valid controllers found! Skipping attachment.");
                    //    Destroy(this.gameObject);
                    //    return;
                    //}

                    //BubbleController visitingController = filteredControllers
                    //    .MinBy(controller => Vector2.Distance(controller.transform.position, transform.position));

                    if (!RCGSoundSetting.Instance.IsSoundMuted)
                    {
                        RCGSoundSetting.Instance.PlaySFX(ballLandAudioClip);
        ...
        private void OnCollisionEnter2D(Collision2D collision)
        {
            ...
            if (!_controller.isInitialize)
            {
                if (!RCGSoundSetting.Instance.IsSoundMuted)
                {
                    RCGSoundSetting.Instance.PlaySFX(ballBoundAudioClip);
                    Debug.Log("tuandeptrai " + collision.gameObject.name);
                }
            }
