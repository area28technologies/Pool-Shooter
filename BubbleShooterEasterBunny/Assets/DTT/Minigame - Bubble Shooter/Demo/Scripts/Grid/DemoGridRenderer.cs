...
namespace DTT.BubbleShooter.Demo
{
    ...
    public class DemoGridRenderer : MonoBehaviour
    {
        ...
        public void Render(Bubble bubble, BubbleController controller, Vector3 position)
        {
            ...
            _renderers[bubble.GetType()].Render(bubble, controller, BubbleSprites, _manager.Config.ColorConfiguration);
        }
        ...
        private void Generate()
        {
            ...
            for (int x = 0; x < _manager.Grid.Width; x++)
            {
                for (int y = 0; y < _manager.Grid.RealHeight; y++)
                {
                    ...
                    controller.Initialize(_manager, this, bubble, true);
            ...
            scale = m_cameraController.TargetSize * (1f / m_cameraController.TargetAspect) * 1.5f;
        ...
        private void Redraw(IEnumerable<HexagonCell> updatedCells, HexagonRelativityMode mode, bool instant, bool playEffect = true)
        {
            StartCoroutine(RedrawCoroutine(updatedCells, mode, instant, playEffect));
        }
        ...
        private async void InitiateHide(BubbleController controller, bool playEffect = true)
        {
            // em play effect ở dòng 2 dòng này nha anh
            if (playEffect)
                controller.PlayParticalSystem();
        ...
        /// <param name="mode">The <see cref="HexagonRelativityMode"/> currently used to properly display the grid.</param>
        /// <param name="instant">Whether to instantly update the grid or update each cell one-by-one.</param>
        private IEnumerator RedrawCoroutine(IEnumerable<HexagonCell> updatedCells, HexagonRelativityMode mode, bool instant, bool playEffect = true)
        {
             if (!instant)
             {
             ...
             foreach (HexagonCell updatedCell in updatedCells)
             {
                 ...
                 {
                     InitiateHide(controller, playEffect);
                 }
                 else
                 {
                     controller.SpriteRenderer.sprite = null;
