...
namespace DTT.BubbleShooter.Demo
{
    ...
    public class ScoreManager : MonoBehaviour
    {
        ...
        private void Initialize()
        {
            ...
            _text.text = _score.ToString();
            _comboText.text = $"Combo: {comboCount.ToString()}";
            _coinText.text = coinCount.ToString();

            timeToChange = _manager.Config.TimeToChange[index];
            _manager.OnChangeRowGenerationCodition?.Invoke(index);
            index++;

            //if (index >= _manager.Config.TimeToChange.Length)
            //{
            //    index = 0;
            //}
        }
        ...
        private void Update()
        {
            ...
            if (timeCount > timeToChange && timeToChange > 0 && index < _manager.Config.ShotsTillNewRow.Length)
            {
                _manager.OnChangeRowGenerationCodition?.Invoke(index);
