...
namespace DTT.BubbleShooter
{
    ...
    public abstract class BubblePool
    {
        ...
        public List<Bubble> PickBubble(bool isSwap = false)
        {
            if (p_sets.Count == 0)
            {
            ...
            //if (randomSet == null)
            //    bubblesSwap[0] = p_sets[Random.Range(0, p_sets.Count)].GenerateBubble();

            while (bubblesSwap.Count < 2)
            {
                bubblesSwap.Add(randomSet.GenerateBubble());
            }

            if (bubblesSwap.Count > 2)
            {
                Debug.Log(bubblesSwap.Count);
            }

            if (isSwap)
            {
                Debug.Log("BubbleCount: " + bubblesSwap.Count);
            ...
            else
            {
                ...
                bubblesSwap[1] = randomSet.GenerateBubble();
            }
